...
namespace dmtcp
{
  ...
  class DmtcpCoordinator
  {
    public:
      ...
      string handleGetCommand(char cmd, DmtcpMessage* reply = NULL);
      string handlePostCommand(char cmd, char* data, DmtcpMessage* reply = NULL);
      void handleUserCommand(char cmd, DmtcpMessage* reply = NULL);
      string getRestStatusStr(size_t numPeers, bool isRunning);
