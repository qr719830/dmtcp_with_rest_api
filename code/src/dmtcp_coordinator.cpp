...
#define ERR_PAGE "<html><head><title>Error</title></head><body>Bad data</body></html>"
...
//get dmtcp status as a string
string DmtcpCoordinator::getRestStatusStr(size_t numPeers, bool isRunning)
{
  ostringstream o;
  o << "Status..." << std::endl
    << "Host: " << coordHostname
...
string DmtcpCoordinator::handlePostCommand(char cmd,char* data,  DmtcpMessage* reply /*= NULL*/ )
{
  string repStr;
  ...
  switch ( cmd ){
  	case 'i': case 'I':
  	{
    		...
    		if ((atoi(data)==0)&&(data_str!="0"))
			repStr = "wrong interval format.";
    		else 
    		{
    			updateCheckpointInterval(atoi(data));
    			repStr = "Checkpoint interval has been set to " + intToStr(theCheckpointInterval);
    		}
    		break;
  	}
  	default:
	{
    		repStr = "unhandled post command"+cmd;
    		if (reply != NULL){
      			reply->coordCmdStatus = CoordCmdStatus::ERROR_INVALID_COMMAND;
    		}
	}
  }
  printf("%s", repStr.c_str());
  return repStr;
}

string  DmtcpCoordinator::handleGetCommand(char cmd, DmtcpMessage* reply /*= NULL*/)
{
  string repStr;
  if (reply != NULL) reply->coordCmdStatus = CoordCmdStatus::NOERROR;
  ...
  case 'b': case 'B':  // prefix blocking command, prior to checkpoint command
    blockUntilDone = true;
    repStr = "blocking checkpoint beginning...";
    break;
  case 'x': case 'X':  // prefix exit command, prior to checkpoint command
    repStr =  "Will exit after creating the checkpoint...";
    exitAfterCkpt = true;
    break;
  case 'c': case 'C':
    repStr = "checkpointing...";
    if(startCheckpoint()){
  ...
  case 'i': case 'I':
    ...
    if (theCheckpointInterval == 0)
      repStr= "Current Checkpoint Interval: Disabled (checkpoint manually instead)";
    else
      repStr = "Current Checkpoint Interval:"+intToStr(theCheckpointInterval);
    if (theDefaultCheckpointInterval == 0)
      repStr = "Default Checkpoint Interval: Disabled (checkpoint manually instead)\n";
    else
      repStr = "Default Checkpoint Interval:" +intToStr(theDefaultCheckpointInterval);
    break;
  case 'l': case 'L':
  case 't': case 'T':
    repStr = "Client List:";
    repStr = repStr +"#, PROG[virtPID:realPID]@HOST, DMTCP-UNIQUEPID, STATE";
    for (size_t i = 0; i < clients.size(); i++) {
      repStr = repStr + intToStr(clients[i]->clientNumber());
      repStr = repStr +  ", " + clients[i]->progname();
      repStr = repStr +  "[" + intToStr((int)clients[i]->identity().pid())+ ":" + intToStr((int) clients[i]->realPid());
      repStr = repStr +  "]@" + clients[i]->hostname();
#ifdef PRINT_REMOTE_IP
      repStr = repStr +  "(" + clients[i]->ip() + ")";
#endif
      repStr = repStr +   ", " +clients[i]->identity().toString();
      repStr = repStr +   ", " +clients[i]->state().toString();
    }
    break;
  case 'q': case 'Q':
  {
  ...
  case 'k': case 'K':
    ...
    broadcastMessage ( DMT_KILL_PEER );
    repStr = "killing all connected ";
    break;
  case 'h': case 'H': case '?':
    repStr= theHelpMessage;
    break;
  case 's': case 'S':
  {
    ...
    if (reply != NULL) {
     ...
     repStr =  getRestStatusStr(s.numPeers, running);
    }
    break;
  }
  case ' ': case '\t': case '\n': case '\r':
...
static int url_handler(void *cls,
	               ...
	               const char *method,
	               const char *version,
	               const char *upload_data, size_t *upload_data_size, void **preserved_struct)
{
  // A http request might contain many calls to this function(i.e., for a PUT / POST operation with plenty of upload data). 
  // preserved_struct can be preserved by MHD for future calls for this request. 
  // Initially set to NULL, that this callback can set to some address.
  // preserved_struct is also used to decide if the current call to this function is the first call to this function.
  // A http request will be handled this way: 
  // 1. first time call to this function, create preserved_struct, but not look into accutal request info. 
  // 2. calls after that will be handled, their url and data will be examined.


  // first time call to this funtion
  if (NULL == *preserved_struct)
  {
    char **a;
  ...
  //not first call, examine accutal data
  if (0 == strcmp(method, "GET"))
  // get method request can be handled in one call to this function.
  {
    const char *page;
    const char *urlErrorPage = "<html><body>Wrong url format. Usage: ip_addr:port/command. Try /h for help.</body></html>";
    struct MHD_Response *response;
    int ret;
    printf("%s\n", method);
    printf("%s\n", url);
    std::string url_str(url);
    if ((url_str.length() != 2) || (url[0]!='/'))
	page = urlErrorPage;
    else
	page = prog.handleGetCommand(url[1]).c_str();
	...
	//special operation for q command
	if ((url_str.length() == 2) && ((url[1] == 'q')|| (url[1] == 'Q')))
		exit(0);
	return ret;
    }

    //for post method, there could be several calls to this function. If one call doesn't have any data, that will be the end of post calls
	...
	if (*upload_data_size != 0)
	{
	  ...
	  *data = (char*) malloc(*upload_data_size*sizeof(char));
	  strcpy(*data, upload_data);
	  *preserved_struct = data;
	  *upload_data_size = 0;
	  return MHD_YES;
	}
	//post ends
	else
	{
    	  ...
    	  if ((url_str.length() != 2) || (url[0]!='/'))
	  {
          ...
          {
        	page = prog.handlePostCommand(url[1], *(char **)(*preserved_struct)).c_str();
	  }
	  response = MHD_create_response_from_buffer(strlen(page), (void *)page,
				MHD_RESPMEM_PERSISTENT);
	  ret = MHD_queue_response(connection, MHD_HTTP_OK, response);
...
//calculate certificate file size, helper function of load_cert
static long get_cert_size(const char *cert_path)
{
	FILE *fp;
...
//function for loading ssl certificate files
static char *load_cert(const char *cert_path)
{
	FILE *fp;
	char *buffer;
	long cert_size;
	...
	if (cert_size != fread(buffer, 1, cert_size, fp))
	{
		free(buffer);
		buffer = NULL;
	}
...
void DmtcpCoordinator::eventLoop(bool daemon)
{
  ...
  //declear ssl key files
  char *key_pem;
  char *cert_pem;

  key_pem = load_cert ("server.key");
  cert_pem = load_cert ("server.pem");

  //setup https server at port 1234
  struct MHD_Daemon *d_https;
  d_https = MHD_start_daemon(MHD_USE_SELECT_INTERNALLY|MHD_USE_DEBUG|MHD_USE_SSL,
                        1234,
                        0, 0, &url_handler,NULL,MHD_OPTION_HTTPS_MEM_KEY, key_pem,MHD_OPTION_HTTPS_MEM_CERT, cert_pem, MHD_OPTION_END);
  if (d_https  == 0){
       printf("https server starting failed\n");
  }

  //setup http server at port 8888
  struct MHD_Daemon *d_http;

  d_http = MHD_start_daemon(MHD_USE_SELECT_INTERNALLY|MHD_USE_DEBUG,
                        8888,
                        0, 0, &url_handler,NULL, MHD_OPTION_END);
  if (d_http == 0){
      printf("http server starting failed\n");
  }
